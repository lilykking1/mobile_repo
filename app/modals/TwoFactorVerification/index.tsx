...
export interface TwoFactorVerificationProps {
  route: {
    params: RootRoutes['TwoFactorVerification'];
  };
}

const TwoFactorVerification: FC<TwoFactorVerificationProps> = ({ route }) => {
  const { screenToNavigate, screenToNavigateParams } = route.params;
  ...
  const handleContinuePress = useCallback(() => {
    ...
    if (screenToNavigate) {
      navigation.goBack();
      navigation.navigate(screenToNavigate, screenToNavigateParams);
    } else {
      navigation.goBack();
    }
  }, [handleSubmit, navigation, screenToNavigate, screenToNavigateParams]);
        ...
        <View style={styles.container}>
          ...
          >
            ...
            <View style={styles.subContent}>
              ...
              <Typography
                ...
                style={styles.title}
              >
                {translate('modals.twoFactorVerification.title')}
              </Typography>
              <Typography size="body1" style={styles.instructions}>
                {translate('modals.twoFactorVerification.text.instructions')}
              </Typography>
          ...
          <View style={styles.footer}>
            <Button
              variant="primary"
              block
              label={translate('modals.twoFactorVerification.action.continue')}
